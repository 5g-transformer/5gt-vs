 ...
 *
 */
public class NfvoOperationPollingTask implements Runnable {
	
	private static final Logger log = LoggerFactory.getLogger(NfvoOperationPollingTask.class);
	
	private NfvoOperationPollingManager pollingManager;
	
	private NsLcmProviderInterface nfvoService;
	
	private NfvoNotificationInterface listener;
	
	public NfvoOperationPollingTask(NfvoOperationPollingManager pollingManager,
			NsLcmProviderInterface nfvoService,
			NfvoNotificationInterface listener) {
	...
	public void run() {
		...
		Map<String, PolledNfvoOperation> polledOperations = pollingManager.getPolledOperationsCopy();
		...
		synchronized (polledOperations) {
			for (Map.Entry<String, PolledNfvoOperation> e : polledOperations.entrySet()) {
				PolledNfvoOperation operation = e.getValue();
	...
	private boolean checkOperationSuccessfulResult(PolledNfvoOperation operation) {
