...
public class VsRecordService {
	...
	public List<NetworkSliceInstance> getUsableSlices(String tenantId, String nsdId, String nsdVersion, String dfId, String instantiationLevel) {
	    log.debug("Searching usable network slice instances matching tenant, nsdid, nsdVersion, dfId and instantiationlevel");
		List<NetworkSliceInstance> networkSliceInstances = nsInstanceRepository.findByTenantIdAndNsdIdAndNsdVersionAndDfIdAndInstantiationLevelId(tenantId, nsdId, nsdVersion, dfId, instantiationLevel);
	    if(networkSliceInstances.isEmpty()){
            log.debug("Empty list, ignoring NSD version.");
            networkSliceInstances = nsInstanceRepository.findByTenantIdAndNsdIdAndAndDfIdAndInstantiationLevelId(tenantId, nsdId, dfId, instantiationLevel);
        }
